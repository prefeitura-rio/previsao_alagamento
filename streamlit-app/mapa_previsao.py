...
st.markdown("""
Faça upload de um arquivo csv com o mesmo formato encontrado em rj-cor-dev.clima_pluviometro.main_table_fields_1H_mais_frequentes, adicionando uma coluna com nome "predicted" com o resultado do seu modelo e visualize a performance do modelo.""")
...
if uploaded_file is not None:
    ...
    resultados.loc[len(resultados)] = ("Acurácia: ", accuracy_score(y, y_pred_rf), accuracy_score(y, main_table['predicted']))
    resultados.loc[len(resultados)] = ("R²: ", r2_score(y, y_pred_rf), r2_score(y, main_table['predicted']))
    resultados.loc[len(resultados)] = ("Erro: ", mean_squared_error(y, y_pred_rf), mean_squared_error(y, main_table['predicted']))
    resultados.loc[len(resultados)] = ("Precisão: ", precision_score(y, y_pred_rf), precision_score(y, main_table['predicted']))
    resultados.loc[len(resultados)] = ("Recall: ", recall_score(y, y_pred_rf), recall_score(y, main_table['predicted']))
    resultados.loc[len(resultados)] = ("MCC: ", matthews_corrcoef(y, y_pred_rf), matthews_corrcoef(y, main_table['predicted']))
