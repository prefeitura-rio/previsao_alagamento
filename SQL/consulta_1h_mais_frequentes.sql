...
WITH
    ...
    frequencia_h3 AS (
        SELECT
            id_h3,
            COUNT(*) AS frequencia
        FROM `rj-cor-dev.clima_pluviometro.ocorrencias_alagamento`
        GROUP BY id_h3
    ),
    ...
    FROM (
        WITH centroid_h3 AS (
            ...
            INNER JOIN frequencia_h3
                ON frequencia_h3.id_h3 = id
            WHERE frequencia_h3.frequencia >= 20
        ),
    ...
    FROM (
        SELECT 
            *,
            ROW_NUMBER() OVER (PARTITION BY DATETIME_TRUNC(data_update, HOUR), id_h3 ORDER BY hora_diff) AS ranking_hora
        FROM
            media_agrupada
    ) as media_agrupada
